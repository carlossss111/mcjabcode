...
//gcc -c -fPIC -I. -I${JAVA_HOME}/include -I${JAVA_HOME}/include/linux -Ijabcode -Ijabcode/include -O2 -std=c11 JabEncoder.c -o JabEncoder.o
//gcc -shared -fPIC JabEncoder.o -Ljabcode/build -ljabcode -Ljabcode/lib -ltiff -lpng16 -lz -lm -O2 -std=c11 -o JabEncoder.so -lc
JNIEXPORT void JNICALL Java_uk_ac_nottingham_hybridarcade_encoding_JabEncoder_printJab
  (JNIEnv* env, jobject javaObject, jbyteArray byteData, jbyteArray filePath){

	jsize length = (*env)->GetArrayLength(env, byteData);
	jbyte *dataPtr = (*env)->GetByteArrayElements(env, byteData, 0);
	jbyte *pathPtr = (*env)->GetByteArrayElements(env, filePath, 0);

	printf("Recieved: %s %s\n", dataPtr, pathPtr);
    char *args[] = {"JabEncoder", "--input", dataPtr, "--output", pathPtr};
    main(5, args);

	(*env)->ReleaseByteArrayElements(env, byteData, dataPtr, 0);
	(*env)->ReleaseByteArrayElements(env, filePath, pathPtr, 0);
}
